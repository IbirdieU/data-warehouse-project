...
===============================================================================
Purpose:
    Performs the full ELT load from the 'silver' schema into the 'gold' Star
...
BEGIN
    ...
    -- =========================================================================
    -- DIMENSION 2: gold.dim_customers  (Identity Resolution)
    -- Source  : silver.olist_cust  JOIN  silver.olist_ord
    -- Grain   : one row per unique person (customer_unique_id)
    -- Logic   : ROW_NUMBER() partitioned by customer_unique_id, ordered by
    --           purchase_ts DESC picks the most recent order per person.
    --           This ensures zip_code_prefix, city, state reflect the
    --           customer's Current Location (latest known address).
    -- Mapping : cst_cust_unique_id -> customer_unique_id  (Natural Key)
    ...
    SELECT
        cst_cust_unique_id,    -- NK: one row per real-world person
        cst_zip_code_prefix,   -- Latest shipping ZIP code prefix
        cst_city_std,          -- Latest standardised city name (NOT cst_city_raw)
        cst_state              -- Latest state abbreviation
    FROM (
        SELECT
            c.cst_cust_unique_id,
            c.cst_zip_code_prefix,
            c.cst_city_std,
            c.cst_state,
            ROW_NUMBER() OVER (
                PARTITION BY c.cst_cust_unique_id
                ORDER BY o.ord_purchase_ts DESC
            ) AS rn
        FROM silver.olist_cust c
        INNER JOIN silver.olist_ord o
            ON o.ord_cust_id = c.cst_cust_id
    ) ranked
    WHERE rn = 1;
    ...
    SELECT
        ...
        -- Sourced from silver.olist_cust via the order-specific customer_id,
        -- preserving the exact shipping address for each transaction.
        c.cst_zip_code_prefix       AS shipping_zip_code_prefix,
        c.cst_city_std              AS shipping_city,
    ...
    -- Customer source: provides order-specific shipping location (Point-in-Time DD columns)
    INNER JOIN silver.olist_cust c
        ON c.cst_cust_id = o.ord_cust_id

    -- Dimension SK lookups — INNER JOIN acts as a data quality gate
    -- dim_customers is now keyed on customer_unique_id
    INNER JOIN gold.dim_customers dc
        ON dc.customer_unique_id = c.cst_cust_unique_id
